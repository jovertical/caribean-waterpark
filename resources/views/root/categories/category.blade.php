    ...
    <td>
        <span class="d-flex" style="overflow: visible;">
            ...
            <form method="POST" action="{{ route('root.categories.destroy', $category->id) }}" class="confirm"
                ...
                <button type="submit" data-toggle="modal" data-target="#deleteCategory" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Delete category"><i class="la la-trash"></i></button>
            </form>
        </span>
    </td>
</tr>