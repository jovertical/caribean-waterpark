...
@section('content')
    <div class="m-portlet">
        ...
        <form method="POST" action="{{ route('root.users.update', $user) }}" id="form-user-update" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed m-form--state">
            {{ method_field('PATCH') }}
            {{ csrf_field() }}
